...
- Auth hardening foundation: refresh-token rotation with reuse detection
  (`lib/auth/tokens.server.ts`), strict `__Host-` session cookie policy and
  signed double-submit CSRF (`SECURITY.md`).
- Response caching by route class plus TTL `memoize` for server compute, applied
  at the worker entry so SSR HTML also gets headers (`PERFORMANCE.md`).
- Production error tracking with breadcrumbs, batched beacon delivery and a
  rate-limited `/api/public/errors` ingest that forwards to Sentry.
- Hidden source maps, `x-app-release` stamping and `bun run release` /
  `sourcemaps:upload` scripts (`RELEASE.md`).
- Premium visual pass: aurora backdrop, glass surfaces, hairline edges, sheen
  hero, gradient display type and reduced-motion support.

### Fixed

- Security and cache headers were set only in `requestMiddleware` and never
  reached SSR document responses.
